...
use Obsidian\Exceptions\Web\Style\ColorException;
...
class Color
{
  ...
  private function color(string $name): self
  {
    if (!$color = self::__colors()[$name] ?? null) :
      throw new ColorException("Whoops...\"{$name}\" is not a valid CSS color name");
    endif;
  ...
  private static function __colors(string $find = null): array
  {
    ...
    throw new ColorException("Whoops...\"{$find}\" is not a valid CSS color name");
  }
